...
with open(filePath) as fread:
	...
	if not found:
		...
		resultsummary2 = "This device does not have SSH"
		...
		try:
			...
			if os.path.exists(userNames_list_file) and os.path.exists(password_list_file):
				command="hydra -I -F -L {} -P {} -s {} ssh://{}".format(userNames_list_file, password_list_file, sshPort, IP)
				...
				for line in resultsLines:
					if f"[{sshPort}][ssh] host: {IP}" in line: #the line that indicate the password is found
				...
				if credientialsFound:
					...
					resultsummary2 = f"The SSH credientials found is {user}:{password}"				
				else:
...
filePath= os.path.join(resultsDir,DeviceMAC,"SSHBruteForce_Summary" )
